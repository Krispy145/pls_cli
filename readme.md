...
The app brick is automatically generated by the `./packages/app_brick_generator/` tool. The source of the app brick can be found in the `./packages/app/template/` folder. The app_template is a fully working flutter app and can be run while developing to test new features and additions. 

Once the app_template is ready to be used in the create command it needs to be transpiled to a brick. This is where all the references to "app_template" are replaced with moustache syntax ready for the `create` command.

To transpile the app_template run the following in the cli root directory (you can see the underlying command in the pubspec):
